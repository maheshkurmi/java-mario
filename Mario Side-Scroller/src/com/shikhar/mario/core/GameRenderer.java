...
public class GameRenderer {
    ...
    private static Image[] fontFront, fontShadow;
    
    private static Image waterWave;
    ...
    public void setBackground(BufferedImage background) {
        ...
        fontFront=new SpriteMap("items/font1.png",96,1).getSprites();
        fontShadow=new SpriteMap("items/font2.png",96,1).getSprites();
        waterWave=ImageManipulator.loadImage("items/Water_Wave2.png");
    }
    ...
    public void draw(Graphics2D g, TileMap mainMap, TileMap backgroundMap, TileMap foregroundMap,int screenWidth, int screenHeight, boolean editMode) {
        ...
        if (background != null && !editMode) {
            ...
            g.drawImage(background, x-1, y-1, null);
        }
        ...
        if(lastLife != player.getHealth()); {
    		...
    		for(int i=0; i<mainMap.waterZones().size();i++){
    			...
    			if (!r.isEmpty()){
    				...
    				//g.drawImage(waterWave,r.x*16+offsetX,r.y*16-waterWave.getHeight(null)+offsetY,null);
    			}
    		}
	...
	private void drawGrids(Graphics2D g ,int screenHeight, int screenWidth,int maxHeight,int maxWidth) {
	 ...
	 public static void drawStringDropShadow(Graphics g, String text, int x, int y)
	    {
	        drawString(g, text, x*8+5, y*8+5, true);
	        drawString(g, text, x*8+4, y*8+4, false);
	    }

	    private static void drawString(Graphics g, String text, int x, int y, boolean shadow)
	    {
